...
<?php
if(isset($_POST['v'])){    // في حالة النقر على زر النتيجة سيتم طباعة تقرير بدرجات الطالب في هيئة جدول
  $ss=mysql_query("SELECT student_id  from information where student_id='".$_POST['std']."' and p_id='".$_POST['pid']."'  ")or die(mysql_error());  if(mysql_num_rows($ss)>0){ //التأكد من أن رقم الهوية و رقم الطالب موجودين فعلا في قاعدة البيانات
...
echo "الرجاء إدخال البيانات في جميع الحقول";

}else {echo "خطأ في البيانات المدخلة";}
}
